...
def get_html_bs141(year, month, day):
	...
	path = "./" + year + "/" + month + "/" + w + "week_bs141.html"
...
def get_html_bs142(year, month, day):
	...
	w = util.get_week_of_year(year, month, day)
	path = "./" + year + "/" + month + "/" + w + "week_bs142.html"
	if os.path.exists(path):
		f = open(path, 'r')
...
def getCategoryCode(title_name):
	if "Music" in title_name or "MUSIC" in title_name or "サウンド" in title_name or "SOUND" in title_name or "Sound" in title_name or "名曲" in title_name or "歌謡曲" in title_name or "最新曲" in title_name or "ハヤミミ" in title_name or title_name == "ヒーリングデイズ" or title_name == "J−POPヒストリー" or title_name == "Chill out Time" or title_name == "オトノヨリミチ":
		return "104100"
	elif "BASEBALL" in title_name or "侍ジャパン" in title_name or "野球" in title_name or "クライマックスシリーズ" in title_name:
		return "101101"
	elif "FIFA" in title_name or "WEリーグ" in title_name:
		return "101102"
		...
		return "101108"
	elif "パシフィックネーションズカップ" in title_name:
		return "101115"
	elif "お得α" == title_name or "ジェムサローネ" in title_name or "Shopping" in title_name:
		return "102104"
