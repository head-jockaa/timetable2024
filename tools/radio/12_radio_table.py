...
def get_timetable_nhk(year, month, day, area):
	...
	for station_tag in station_tags:
		...
		last_programs_interval[station_tag] = 0
		...
		for item in items:
			...
			if station_tag not in nhk_stations:
				nhk_stations[station_tag] = []
			...
			bar = create_diff(station_tags[idx], start_time, types, name_id, chapter_id, splited_by_space, desc_id)
			last_programs_interval[station_tag] = interval

			nhk_stations[station_tag].append(bar)
	...
	# 最後の番組の放映時間
	for station_tag in nhk_stations:
		nhk_stations[station_tag][-1] = nhk_stations[station_tag][-1] + ":" + str(last_programs_interval[station_tag])
...
def get_timetable_bs531(year,month,day):
	...
	for program_part in program_parts:
		...
		types = ""
		bar = create_diff("HOUSOUDAIGAKU", start_time, types, name_id, chapter_id, splited_by_space, desc_id)
		programs.append(bar)

	# 最後の番組の放映時間
	programs[-1] = programs[-1] + ":45"

	return {"name":"HOUSOUDAIGAKU", "programs":programs}
		...
		for day in radio_util.days:
			...
			for nhk_area in radio_util.nhk_areas:
				if (int)(radio_util.year) >= 2025 or (int)(radio_util.year) == 2024 and (int)(month) >= 5 or (int)(radio_util.year) == 2024 and (int)(month) == 4 and (int)(day) == 30:
					...
					result = get_timetable_nhk_old(radio_util.year,month,day,nhk_area)
				if result == None:
					continue
			...
			if (int)(radio_util.year) >= 2025 or (int)(radio_util.year) == 2024 and (int)(month) >= 4:
				result = get_timetable_bs531(radio_util.year,month,day)
				if result != None:
					if not isFirstStation:
						outfile.write(",\r\n")
					outfile.write(result["name"] + ":\"")
